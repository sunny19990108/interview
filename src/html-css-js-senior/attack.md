...
跨站点伪造请求，比如 在 A 网站，诱导用户跳转到 B 网站，并向 A 网站接口发请求
请看下面的故事
...
预防：
浏览器禁止第三方 cookie (setCookie: SameSite='Lit')
关键接口使用短信验证码等双重验证

### 点击劫持 Click jacking
诱导用户点出 iframe, iframe 内嵌入其他网页并设置成透明盖在当前页面上
...
预防： 
使用 js 判断 iframe 的域名是不是和自己页面域名相同
...
## DDos
Distributed denial-of-service 分布式拒绝服务，是对服务端的攻击

通过大规模的网络流量淹没目标服务器或其周边基础设施，以破坏目标服务器、服务或网络正常流量的恶意行为。<br>
类似于恶意堵车，妨碍正常车辆通行。
比如：通过一些病毒、木马 种到用户手机上（大批量的，比如一百万个用户），然后操控手机同时访问某个网站，使网站瘫痪
